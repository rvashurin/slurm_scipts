#!/bin/bash
#SBATCH --job-name=mistral_deen_sentsar
...
#SBATCH --parsable
#SBATCH --output=/fsx/homes/Roman.Vashurin@mbzuai.ac.ae/logs/mistral_deen_sentsar
...
PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python HF_HOME=/fsx/hyperpod-output-artifacts/'AROA6GBMFKRI2VWQAUGYI:Roman.Vashurin@mbzuai.ac.ae' HYDRA_CONFIG=`pwd`/examples/configs/polygraph_eval_wmt19_deen_sentsar.yaml polygraph_eval batch_size=1 model.path=mistral-community/Mistral-7B-v0.2 subsample_eval_dataset=2000 deberta_batch_size=1 cache_path=/fsx/hyperpod-output-artifacts/'AROA6GBMFKRI2VWQAUGYI:Roman.Vashurin@mbzuai.ac.ae'/polygraph_out
